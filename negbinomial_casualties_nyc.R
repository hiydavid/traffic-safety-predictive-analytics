...
data <- read_csv("C:/Users/nbranham/Documents/NYU_MSBA/Capstone_Files/Master.1.20.2019.csv")
...
collisions <- data %>%
  group_by(City, GEOID) %>%
  summarise(TotalCasualties=sum(TotalCasualties))
...
m1 <- glm.nb(TotalCasualties~.,data = modeldata2)
summary(m1)
...
# Calculate residuals
modeldata2$residuals <- modeldata2$TotalCasualties-modeldata2$predictions

# look at the RMSE 
modeldata2 <- modeldata2[complete.cases(modeldata2), ]
rmse(modeldata2$TotalCasualties, modeldata2$predictions)
# RMSE of 139.731
# AIC of 23156
# this is higher than the mean of 111.5532, sd of 104.5452
...
# Specify the full model using all of the potential predictors
m3 <- glm.nb(TotalCasualties~.,data = modeldata3)
summary(m3)
m3summary <- tidy(m3)
...
rmse(modeldata3$TotalCasualties, modeldata3$predictions)
# RMSE of 139.4696
# AIC of 20072
# this is higher than the mean of 111.5532, sd of 104.5452
...
# Specify the full model using all of the potential predictors
m4 <- glm.nb(TotalCasualties~.,data = modeldata4)
summary(m4)
m4summary <- tidy(m4)
...
rmse(modeldata4$TotalCasualties, modeldata4$predictions)
# RMSE of 139.4695
# AIC of 20069
# this is higher than the mean of 111.5532, sd of 104.5452
...
# ______________________________________________________________________________________________
area <- read_csv("C:/Users/nbranham/Documents/NYU_MSBA/Capstone_Files/data_2010_area.csv")
area <- area[,-(2:5)]
...
new_vars <- read_csv("C:/Users/nbranham/Documents/NYU_MSBA/Capstone_Files/new_vars.csv")
...
# feature out df_features_1 for nyc
# df_features_1 = subset(df_features_1, City=="NYC")
# write.csv(df_features_1, "df_features_1_nyc.csv", row.names = FALSE)
...
# Specify the full model using all of the potential predictors
m5 <- glm.nb(TotalCasualties~.,data = df_features_1) 
summary(m5)
m5summary <- tidy(m5)
...
rmse(df_features_1$TotalCasualties, df_features_1$predictions)
...
# View(df_features_2)
# write.csv(df_features_2, "df_features_2_nyc.csv", row.names = FALSE)

drops4 <- c("GEOID","City","Collisions","TotalDeaths")
df_features_2 <- df_features_2[ , !(names(df_features_2) %in% drops4)]

# Specify the full model using all of the potential predictors
m6 <- glm.nb(TotalCasualties~.,data = df_features_2)
summary(m6)
m6summary <- tidy(m6)
...
rmse(df_features_2$TotalCasualties, df_features_2$predictions)
...
# df_features_4 <- left_join(features_4, collisions, by = "GEOID")
# write.csv(df_features_4, "df_features_4_nyc.csv", row.names = FALSE)
drops5 <- c("GEOID","City")
df_features_4 <- df_features_4[ , !(names(df_features_4) %in% drops5)]

# Specify the full model using all of the potential predictors
m7 <- glm.nb(TotalCasualties~.,data = df_features_4)
summary(m7)
m7summary <- tidy(m7)
m7summary$p.value <- round(m7summary$p.value, 2)
arrange(m6summary, desc(p.value))

# look at the Variance Inflation Factor for m1
m7_vif <- tidy(vif(m7))
names(m7_vif) <- c("Variable","VIF")
m7_vif <- arrange(m7_vif, desc(VIF, Variable))
m7_vif

# Make predictions from the model
df_features_4$predictions <- predict(m7, newdata=df_features_4)

# look at the RMSE 
df_features_4 <- df_features_4[complete.cases(df_features_4), ]
rmse(df_features_4$TotalCasualties, df_features_4$predictions)
