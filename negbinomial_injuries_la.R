...
m1data <- subset(modeldata, City=="LA") # change this to la
m1data$TotalDeaths <- NULL # change this to total injuries if predicting total deaths
...
# Calculate residuals
modeldata2$residuals <- modeldata2$TotalInjuries-modeldata2$predictions
...
# RMSE of 37.33048
# AIC of 9303.4
# this is higher than the mean of 28.15993
...
arrange(fwdstepsummary, desc(p.value)) # AIC of 9250.93 - 21 vars
...
# Specify the full model using all of the potential predictors
m3 <- glm.nb(TotalInjuries~.,data = modeldata3)
summary(m3)
m3summary <- tidy(m3)
...
rmse(modeldata3$TotalInjuries, modeldata3$predictions)
# RMSE of 36.97311
# AIC of 8164.5
# this is higher than the mean of 28.15993
...
# Specify the full model using all of the potential predictors
m4 <- glm.nb(TotalInjuries~.,data = modeldata4)
summary(m4)
m4summary <- tidy(m4)
...
rmse(modeldata4$TotalInjuries, modeldata4$predictions)
# RMSE of 36.97294
# AIC of 8160.9
...
m5_vif <- arrange(m4_vif, desc(VIF, Variable))
m5_vif
...
rmse(df_features_1$TotalInjuries, df_features_1$predictions)
...
# write.csv(df_features_1, "df_features_1.csv", row.names = FALSE)
drops4 <- c("GEOID","City","Collisions","TotalDeaths") # Change this out to TotalInjuries
df_features_2 <- df_features_2[ , !(names(df_features_2) %in% drops4)]

# Specify the full model using all of the potential predictors
m6 <- glm.nb(TotalInjuries~.,data = df_features_2)
summary(m6)
m6summary <- tidy(m6)
...
rmse(df_features_2$TotalInjuries, df_features_2$predictions)
