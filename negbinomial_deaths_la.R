...
m1data$TotalInjuries <- NULL # change this to total injuries if predicting total deaths
...
m1 <- glm.nb(TotalDeaths~.,data = modeldata2)
summary(m1)
...
# Calculate residuals
modeldata2$residuals <- modeldata2$TotalDeaths-modeldata2$predictions

# look at the RMSE 
modeldata2 <- modeldata2[complete.cases(modeldata2), ]
rmse(modeldata2$TotalDeaths, modeldata2$predictions)
# RMSE of 1.565875
# AIC of 2345.4
# this is higher than the mean of 0.6163522
...
arrange(fwdstepsummary, desc(p.value)) # AIC of 2289.42 - 12 vars
...
# Specify the full model using all of the potential predictors
m3 <- glm.nb(TotalDeaths~.,data = modeldata3)
summary(m3)
m3summary <- tidy(m3)
...
rmse(modeldata3$TotalDeaths, modeldata3$predictions)
# RMSE of 1.638716
# AIC of 1609.9
# this is higher than the mean of 0.6163522
...
# Specify the full model using all of the potential predictors
m4 <- glm.nb(TotalDeaths~.,data = modeldata4)
summary(m4)
m4summary <- tidy(m4)
...
rmse(modeldata4$TotalDeaths, modeldata4$predictions)
# RMSE of 1.639278
# AIC of 1606
...
drops3 <- c("GEOID","City","Collisions","TotalInjuries") 
df_features_1 <- df_features_1[ , !(names(df_features_1) %in% drops3)]

# Specify the full model using all of the potential predictors
m5 <- glm.nb(TotalDeaths~.,data = df_features_1)
summary(m5)
m5summary <- tidy(m5)
...
rmse(df_features_1$TotalDeaths, df_features_1$predictions)
...
# write.csv(df_features_1, "df_features_1.csv", row.names = FALSE)
drops4 <- c("GEOID","City","Collisions","TotalInjuries") 
df_features_2 <- df_features_2[ , !(names(df_features_2) %in% drops4)]

# Specify the full model using all of the potential predictors
m6 <- glm.nb(TotalDeaths~.,data = df_features_2)
summary(m6)
m6summary <- tidy(m6)
...
rmse(df_features_2$TotalDeaths, df_features_2$predictions)
